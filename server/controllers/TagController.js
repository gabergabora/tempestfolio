...
class TagController extends ApiController {
  ...
  createTag = (req, res, next) => {
    ...
    if (!tagName)
      return res.status(400).json({ status: 0, message: "client error" });
  ...
  editTag = (req, res, next) => {
    ...
    if (!tagID || !tagname)
      return res.status(400).json("tag id and name are required");

    editTag(tagID, tagname)
      .then((tag) => {
        if (!tag)
          return res.status(404).json({ message: "no tag with id found" });
